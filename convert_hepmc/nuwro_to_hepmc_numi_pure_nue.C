...
void nuwro_to_hepmc_numi_pure_nue(std::string indir , std::string nuwro_file_name , std::string outdir, int eventsperfile=100) {
 ...
 Bool_t          flag_cc;
  ...
  static constexpr Int_t kMaxin = 20;
   static constexpr Int_t kMaxtemp = 20;
   static constexpr Int_t kMaxout = 20;
   static constexpr Int_t kMaxpost = 35;
   static constexpr Int_t kMaxall = 35;
...
//std::cout << outdir + nuwro_file_name + "_" + std::to_string(0) + ".hepmc" << std::endl;
   ...
   _hepmc_file.open(outdir + nuwro_file_name + "_" + std::to_string(0) + ".hepmc");
	...
	//check a neutrino was a nue	
	if(abs(in_pdg[0]) != 12) continue;
